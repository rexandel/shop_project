...
public class OrderService {
        ...
        orders.stream()
            .map(
                order -> {
                  ...
                  List<V1CreateOrderResponse.OrderItem> respItems =
                      items.stream()
                          .filter(item -> item.getOrder().getId().equals(order.getId()))
                          .map(
                              item -> {
                                V1CreateOrderResponse.OrderItem respItem =
                                    new V1CreateOrderResponse.OrderItem();
  ...
  public V1QueryOrdersResponse queryOrders(V1QueryOrdersRequest request)
    ...
    List<V1QueryOrdersResponse.Order> responseOrders =
        orderUnits.stream()
            .map(
                orderUnit -> {
                  ...
                  List<V1QueryOrdersResponse.OrderItem> respItems =
                      orderUnit.getOrderItems().stream()
                          .map(
                              itemUnit -> {
                                V1QueryOrdersResponse.OrderItem respItem =
                                    new V1QueryOrdersResponse.OrderItem();
  ...
  public List<OrderUnit> getOrders(QueryOrdersDalModel model, boolean includeOrderItems)
    ...
    Map<Long, List<OrderItem>> lookup =
    ...
    return orders.stream()
        .map(
            o -> {
              ...
              unit.setOrderItems(
                  lookup.getOrDefault(o.getId(), List.of()).stream()
                      .map(
                          i -> {
